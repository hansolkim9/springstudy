...
class SpringJdbcTest {
    ...
    void saveTest() {
        // gwt 패턴
        // given : 테스트에 주어질 데이터
        Person p = new Person(1000, "천천", 1000);
    ...
    void modifyTest() {
        ...
        //when
        boolean flag = springJdbc.update(p);
        //then
        assertTrue(flag);
    }

    @Test
    @DisplayName("사람 정보를 전체 조회하면 결과 건수는 4건이며, 첫번째 사람의 이름은 칠칠이다.")
    void findAllTest() {
        //given
        ...
        assertEquals("칠칠칠", people.get(0).getPersonName());
    }

    @Test
    @DisplayName("사람 정보를 아이디로 단일조회시 아이디가 300인 사람의 이름은 삼백이이고 나이는 33이다.")
    void findOneTest() {
        //given
        long id = 300;
        //when
        ...
        assertEquals(33, person.getPersonAge());
    }
